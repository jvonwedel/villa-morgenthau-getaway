...
const Hero = () => {
  ...
  return <section className="relative h-screen">
      ...
      <div className="absolute inset-0 w-full h-full">
        {heroImages.map((image, index) => <div key={index} className={`absolute inset-0 w-full h-full bg-cover bg-center transition-opacity duration-1000 ${index === currentImage ? 'opacity-100' : 'opacity-0'}`} style={{
